...
const markerDefs = [
 ...
 { key: "emptybottle", label: "빈병", kind:'thing', points: [ {x:0.595,y:0.446}, {x:0.953,y:0.908} ] },
  {
    key: "draftbeerglass",
    label: "생맥주잔",
    kind: "thing",
...
draftbeer: [ { img: "item_생맥주.jpg", desc: "", video: "맥주 따르기.mp4" } ],  shavedice: [{ img: "item_빙수.jpg", desc: "" }],
  ...
  emptybottle: [
    { img: "item_빈술.jpg", desc: "빈병 모아두는 곳" },
    { img: "item_다찬술.jpg", desc: "다 찬 빈병 박스 두는 곳" }
  ],
...
const foodCategories = [
  ...
  },
  {
    ...
    items: [
      ...
      { name: "망고 막걸리", img: "food_망고막걸리.jpg", recipe: "얼음 150g, 시럽 110g, 막걸리 250g, 우유 250g, 냉동과일 50g, 연유 15g / 갈아서 항아리에 부어 나무국자 올려 잔과 함께 제공" },
    { name: "블루베리 막걸리", img: "food_블루베리막걸리.jpg", recipe: "얼음 150g, 시럽 110g, 막걸리 250g, 우유 250g, 냉동과일 50g, 연유 15g / 갈아서 항아리에 부어 나무국자 올려 잔과 함께 제공" },
    ],
  },
  {
    name: "탕",
    items: [
      ...
      { name: "북촌사골만두전골", img: "food_북촌사골만두전골.jpg", plating: "두부 위 깨", sauce: "1구 종지에 진간장", tools: "국자" },
    { name: "얼큰김치우동탕", img: "food_얼큰김치우동탕.jpg", plating: "우동 건더기", tools: "국자, 집게" },
    ],
  },
  {
    name: "마른안주·전",
  ...
  },
  {
    ...
    items: [
      ...
      { name: "육회 칼비빔면", img: "food_육회비빔면.jpg",},
      {
        name: "항정수육",
        img: "food_항정수육.jpg",
     ...
     { name: "족뱅이", img: "food_족뱅이.jpg", plating: "깨" },
    { name: "냉채족발", img: "food_냉채족발.jpg", plating: "깨" },
      {
        name: "물뱅이",
        img: "food_물뱅이.jpg",
        plating: "소스 가운데 깨",
      ...
      { name: "훈제오리", img: "food_훈제오리.jpg", plating: "무침 위에 깨", sauce: "스텐 종지: 쌈무, 머스타드",
 }, 
      {
        name: "삼겹 비빔면",
        img: "food_삼겹비빔면.jpg",
  ...
  },
  {
    ...
    items: [
      ...
      { name: "닭똥집볶음", img: "food_닭똥집볶음.jpg", plating: "깨, 참기름",         sauce: "참기름·소금후추",
tools: "나무 받침",
},
    ],
  },
  ...
  },
  {
    ...
    items: [
      ...
      { name: "김치볶음밥", img: "food_김치볶음밥.jpg", plating: "깨, 참기름", tools: "나무 받침", },
      {
        name: "짜계치와 파김치",
        img: "food_짜계치.jpg",
     ...
     { name: "치즈스틱", img: "food_치즈스틱.jpg", plating: "양배추 위 케찹·마요, 전체 파슬리", sauce: "1구 종지에 케찹" },
    { name: "감자튀김", img: "food_감자튀김.jpg", plating: "양배추 위 케찹·마요, 전체 파슬리", sauce: "1구 종지에 케찹" },
      {
        name: "뿌링클감자튀김",
        img: "food_뿌링클감자.jpg",
        plating: "양배추 위 케찹·마요",
      ...
      { name: "들기름 베이컨후라이", img: "food_들기름베이컨후라이.jpg", plating: "깨", sauce: "케찹" },
      {
        name: "호떡아이스크림",
     ...
     { name: "설탕토스트", img: "food_설탕토스트.jpg", plating: "파슬리" },
    { name: "토마토", img: "food_토마토.jpg", plating: "파슬리" },
...
function setup() {
  ...
  styleTag.innerHTML = `
    * { -webkit-tap-highlight-color: transparent; -webkit-font-smoothing: antialiased; }
    ...
    .fade-up { animation: fadeUp .28s ease both; }/* PC에서 앱을 폰 폭으로 제한하고 가운데 정렬 */
  ...
  let lastTapTime = 0,
    lastTapIdx = -1;
  canvas.mousePressed(() => {
    if (currentScreen === 'plan') deactivateMarker();
  });
}
...
function drawGlowingMarker(px, py, size, blink) {
  ...
  // 바깥 후광 - 더 넓게
  ctx.shadowColor = `rgba(255,10,30,${1.0 * blink})`;
  ctx.shadowBlur = size * 4.0 * blink;
  fill(255, 12, 32, 110 * blink);
  rect(px, py, size * 1.1, size * 1.1);

  // 몸체
  ctx.shadowBlur = size * 2.0 * blink;
  fill(245, 20, 35, 200 * blink);
  rect(px, py, size, size);

  // 코어
  ctx.shadowBlur = size * 0.8;
  fill(255, 50, 60, 245 * blink);
  rect(px, py, size * 0.72, size * 0.72);

  // 중심 하이라이트
  ctx.shadowBlur = 0;
  fill(255, 210, 205, 230 * blink);
  rect(px, py, size * 0.3, size * 0.3);
...
function createSettingScreen() {
  ...
  [
    { file: "세팅1.jpg", title: "세팅 1 - 손님 오시기 전 (물X)" },
    { file: "세팅2.jpg", title: "세팅 2 - 손님 오시면 물 세팅" },
  ].forEach((it, i) => {
    // 제목
    ...
    // 이미지 (가운데 정렬 래퍼)
    const center = createDiv(); center.parent(pad);
    center.style('text-align', 'center');
    center.style('width', '100%');

    const imgWrap = createDiv(); imgWrap.parent(center);
    imgWrap.class('card-shadow');
    imgWrap.style('display', 'inline-block');
    imgWrap.style('max-width', '100%');
    imgWrap.style('background', C.surface2);
    imgWrap.style('border', `1px solid ${C.line}`);
    imgWrap.style('border-radius', '14px');
    imgWrap.style('overflow', 'hidden');
    imgWrap.style('line-height', '0');

    const imgEl = createImg(it.file, it.title); imgEl.parent(imgWrap);
   imgEl.style('max-width', '102%');
    imgEl.style('max-height', '85vh');
    imgEl.style('width', 'auto');
    imgEl.style('height', 'auto');
    imgEl.style('display', 'block');
    imgEl.elt.onerror = () => { imgEl.style('display', 'none'); const ph = createDiv(); ph.parent(imgWrap); ph.html(phSvg); ph.style('padding', '60px'); };
  });
}
...
function parseDrinkRecipe(recipe) {
  ...
  recipe.split("/").forEach((segRaw) => {
    ...
    if (!seg) return;
    // 믹스잔 단계는 한 줄에 콤마로 나열 (줄바꿈 없음)
    if (/믹스\s*잔/.test(seg)) {
      out.push({ text: seg, kind: 'recipe' });
      return;
    }
    const parts = seg
      .split(",")
...
function preloadFoodImages() {
  foodCategories.forEach(cat => {
    cat.items.forEach(item => {
      if (item.img && !foodImgCache[item.img]) {
        const im = new Image();
        im.decoding = 'async';
        im.src = item.img;
        im.style.position = 'absolute';
